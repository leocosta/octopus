...
`e2e-testing`, `dotnet`, and `review-contracts` activate on file
...
♻️ Refactored the three pre-merge audit skills (`audit-money`, `audit-tenant`, `review-contracts`) to share conventions via `skills/_shared/audit-output-format.md`. Severity format, override cascade, `--write-report` frontmatter, and common errors now live in one file; each SKILL.md keeps only its skill-specific content. Tests were updated to look for conventions across both the skill file and the shared fragment.
...
🐛 Fixes missing descriptions for 10 slash commands in Claude Code's `/` list: `/octopus:implement`, `/octopus:debug`, `/octopus:respond-to-review`, `/octopus:audit-all`, `/octopus:review-contracts`, `/octopus:audit-money`, `/octopus:audit-tenant`, `/octopus:plan-backlog`, `/octopus:launch-feature`, `/octopus:launch-release`. The command templates should ship with **two** frontmatter blocks — an outer Octopus metadata block (stripped at delivery) and an inner Claude-readable block (`description:` + `agent:`, preserved). The newer commands were authored with only the outer block, so `strip_frontmatter` removed the entire header and the delivered files had no description for Claude Code to render.
...
✨ Introduces **bundles** as the primary setup path. The wizard's Quick mode (default) now asks 4–6 yes/no persona questions ("Is this a SaaS product for external customers?", "Does your team produce marketing content?", "Primary backend is .NET?") and maps each positive answer to a curated bundle of skills + roles + rules. Seven bundles ship in v1: `starter` (foundation — `adr`, `feature-lifecycle`, `context-budget`; always included), four intent bundles (`quality-gates` → audit-security + audit-money + audit-tenant + backend-specialist role; `growth` → launch-feature + social-media role; `docs-discipline` → plan-backlog + continuous-learning + tech-writer role; `cross-stack` → review-contracts + backend + frontend specialists), and two stack bundles (`dotnet-api`, `node-api`). Users can declare `bundles: [...]` in `.octopus.yml` and the expansion happens at setup time — a new user never needs to memorize the 13-skill catalog to get a sensible config. Power users still pick Full mode or mix explicit `skills:` / `roles:` on top of bundles; all user-explicit entries are additive (bundles never remove selections).
