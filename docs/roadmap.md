...
| RM-162 | `doc-api` Assess & Plan flow — `--write` becomes an interactive per-artifact plan (`correct` / `recreate` / `create` / `skip`) over spec + integrator reference, derived from the four checks; `create`-from-scratch is first-class; `breaking` annotates the chosen action before it is applied; validate mode gains a read-only Improvement Plan preview | in progress → [Spec](specs/doc-api-assess-and-plan.md) | 2026-07-24 |
