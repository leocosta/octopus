...
| RM-161 | `doc-api` skill + command — on-demand, integrator-facing API contract-fidelity validator and doc generator (code ↔ OpenAPI ↔ business knowledge from ADRs/specs/system-maps); per-version scoping; `--write` updates `openapi.yaml` + integrator reference behind a confirm gate; reuses `audit-grounding`; `docs` bundle; not in `audit-all` | completed | 2026-07-18 |
