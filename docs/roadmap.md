...
- **Priority:** 🔴 High
- **Effort:** low
- **Status:** implemented — `review-session judge|score`, #235
- **Added:** 2026-08-06

RM-175 is the item that makes every quality claim in Cluster 31 falsifiable, and
its cost of entry is an annotated corpus: 15–25h of human judgement that cannot be
generated by the system under test. That price has kept it proposed while four
items shipped on top of unmeasured assumptions — including RM-171, whose CHANGELOG
entry says outright that its calibration is unprovable.

This is the first tier of RM-175, and it costs no annotation project. Since RM-176
every review already persists its findings with origin, severity and anchor; since
RM-171 it also persists what the reflection pass dropped and why. The only missing
piece was a place to put the human's answer. `review-session judge` records one
verdict per finding, `review-session score` aggregates across every record.

The verdict vocabulary carries the design decision that keeps the number honest:

| Verdict | Counts as | Why |
|---|---|---|
| `real` | hit | acted on |
| `wont-fix` | **hit** | a real finding you chose not to act on is still a true positive; scoring it as a miss is the standard way to make a reviewer look worse than it is |
| `false` | miss | the only precision failure |
| `wrong-location` | neither | an anchor failure (RM-170), not a judgement failure — it leaves the denominator rather than polluting it |

Verdicts live in a sidecar TSV beside the record, not inside it: the record is
written without jq so one is still produced in degraded environments, and
rewriting that JSON to add a field would put jq on the write path. Append-only
keeps writes a `printf`; the score folds to last-wins, so correcting a verdict
does not double-count.

Filtered findings are judgeable too, which is the part that pays for itself
immediately: of what the reflection pass dropped, how much was actually real. That
is the one direction of RM-171's calibration measurable without a corpus at all.

- **Token cost:** none — all of it is bash and jq on read paths.
- **Runtime:** none.
- **External deps:** none.

**Benefits:**

- **Ends prompt tuning by opinion, incrementally.** The full harness is still worth
  building; this makes the first number available now, from reviews that were going
  to happen anyway.
- **Measures the filter that is already shipping.** RM-171 blocks and drops findings
  today with no evidence either way. `filtered_wrong` is that evidence.
- **The corpus accumulates as a byproduct.** No annotation sprint, no CI, no API
  budget — the cost is one verdict per finding a human was already reading.

**What it does not give: recall.** The record only contains findings the reviewer
emitted, so it can never report what was missed. Recall needs either RM-175's
annotated corpus or a history-mined one (blame back from bugfix commits to the diff
that introduced the defect) — a separate item, and the honest limit of this one.
