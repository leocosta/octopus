...
- **Context** — a *perennial* node in a tree of arbitrary depth
  ...
  state. Example: `tatame` → `jiu-jitsu`.
- **Project** — a *temporal* effort (start / middle / end), and
  **cross-cutting**: a single project can span several contexts and
  even several workspaces. Example: `pos-activation` crosses both `tms`
  and `pos`. Projects live under `projects/` and link the contexts they
