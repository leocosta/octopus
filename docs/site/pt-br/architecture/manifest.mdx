...
Você não precisa escrevê-lo à mão: no `octopus setup` interativo, ligue
**customize** para abrir uma segunda tela listando todas as skills, roles
e rules dos bundles escolhidos, todas pré-marcadas — o que você desmarcar é
gravado aqui como `exclude:`. O passo funciona tanto no picker `fzf` quanto
no fallback de terminal simples (uma lista numerada onde você informa os
itens a excluir).
