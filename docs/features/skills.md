...
| `respond-to-review` | Active-by-default PR-feedback discipline — verify the critique, ask for evidence on generic comments, separate reasoned vs preference, never performative changes, clarify ambiguity. Third workflow skill in `starter` alongside `implement` and `debug`. | `starter` | [respond-to-review.md](respond-to-review.md) |
