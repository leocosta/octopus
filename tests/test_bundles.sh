...
# starter contributes adr, feature-lifecycle, context-budget, implement
printf '%s\n' "${OCTOPUS_SKILLS[@]}" | sort > /tmp/got_skills.$$
printf '%s\n' adr feature-lifecycle context-budget implement | sort > /tmp/exp_skills.$$
...
expected_skills=(adr feature-lifecycle context-budget implement audit-all security-scan money-review tenant-scope-audit cross-stack-contract)
...
[[ ${#OCTOPUS_SKILLS[@]} -eq 9 ]] \
  || { echo "FAIL: expected 9 skills after bundle expansion, got ${#OCTOPUS_SKILLS[@]}"; exit 1; }
