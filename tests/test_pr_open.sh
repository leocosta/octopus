...
echo "Test 3: body file is passed through to gh pr create"
BODY="$TMPDIR/body.md"
cat > "$BODY" <<'MD'
## What
fixture body
...
MD
: > "$STUB_LOG"
PATH="$STUB_BIN:$PATH" "$CLI" pr-open --target main --body-file "$BODY" > "$TMPDIR/out3.txt" 2>&1
grep -q "BODY_FILE_RECEIVED=$BODY" "$STUB_LOG" \
  || { echo "FAIL: gh pr create did not receive the --body-file path"; cat "$STUB_LOG"; exit 1; }
