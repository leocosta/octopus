...
SKILL_FILE="$SCRIPT_DIR/skills/audit-tenant/SKILL.md"
...
head -n 5 "$SKILL_FILE" | grep -q "^name: audit-tenant$" \
...
grep -q "octopus:audit-tenant" "$SKILL_FILE" \
...
TEMPLATES="$SCRIPT_DIR/skills/audit-tenant/templates"
...
CMD_FILE="$SCRIPT_DIR/commands/audit-tenant.md"
[[ -f "$CMD_FILE" ]] || { echo "FAIL: $CMD_FILE missing"; exit 1; }
head -n 5 "$CMD_FILE" | grep -q "^name: audit-tenant$" \
  || { echo "FAIL: command frontmatter missing"; exit 1; }
echo "PASS: slash command present"

echo "Test 8: wizard includes audit-tenant"
WIZARD="$SCRIPT_DIR/cli/lib/setup-wizard.sh"
grep -E "^[[:space:]]*local items=\(.*audit-tenant.*\)" "$WIZARD" >/dev/null \
  || { echo "FAIL: audit-tenant not in items array"; exit 1; }
