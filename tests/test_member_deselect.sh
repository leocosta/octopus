...
# RM-146 — picker member-deselect. The fzf phase-2 itself is interactive (not
# unit-testable), so this covers the pure pieces: the member union the picker
# offers, that the manifest writer emits `exclude:`, and that the excluded
# members are actually dropped end-to-end via _apply_excludes. Grep/exit-code.
set -uo pipefail
