...
description: >
  ...
  operator-run; interim in the quality bundle, final in tech-lead (RM-096).
triggers:
...
## The source of truth — `fleet.yml` (D3)
...
- **Adoption tier** — see D2.

## Adoption tiers — the legacy ratchet (D2)
...
## Merge policy — the migration path (D4)
...
- **`audit-fleet` (RM-094)** — detect → remediate: shares the repo-list
...
- **`enforce-ide` / `enforce-precommit`** — the T1/T2 layers, including the
  workspace config-template precedence (D5).
