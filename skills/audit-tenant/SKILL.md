...
```bash
octopus audit-scope audit-tenant --base <base> --ref <ref>
```

Branch on the marker it prints, and do not second-guess it:

- `skip` — no tenant-scoped files changed. Print the reason and stop.
...
- `scoped` — audit **only** the diff that follows, then persist the result:
  `octopus audit-scope audit-tenant --write <key> --from <report-file>`
